...
button5_rect = pygame.Rect(button4_rect.right + button_gap, 400, button_width, button_height)  # Flèche
...
def update_fleche():
    """
    Mode flèche : simule une flècle qui se déplace de gauche à droite de l'écran
    """
    current_pos = 0
    # Dessine une flèche
    while current_pos + 20 < matrix_height:
        led_matrix[current_pos + 20][matrix_width//2 -1] = (255, 255, 255)
        for i in range(1,9):
            led_matrix[current_pos + 20 - i][matrix_width//2 - 1 - i] = (255, 255, 255)
            led_matrix[current_pos + 20 - i][matrix_width//2 - 1 + i] = (255, 255, 255)
        current_pos += 1
...
def draw_buttons(surface, font):
    ...
    pygame.draw.rect(surface, (120, 0, 120), button5_rect)  # bouton flèche (violet)
    ...
    text5 = font.render("Flèche", True, (255, 255, 255))
